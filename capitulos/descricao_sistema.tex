...
\begin{table}[H]
    ...
    \begin{tabular}{c|c}
    \textbf{Arquitetura} & \textbf{AVR RISK} \\
    \hline
...
O Arduino Mega será utilizado para calcular a posição do carro (utilizando o \textit{encoder}), controlar a velocidade e o sentido de giro do motor CC (utilizando a Ponte H) e receber os dados dos dois sensores de fim de curso.
...
\begin{table}[H]
    ...
    \textbf{Tensão (V)}  & \bm{$k_0 \mathrm{(\frac{rad}{Vms}})$} & \bm{$\tau$} \textbf{(s)} & \bm{$B_c \mathrm{(\frac{Ns}{m^2})}$} & \textbf{M (kg)}\\[3pt]  \hline
